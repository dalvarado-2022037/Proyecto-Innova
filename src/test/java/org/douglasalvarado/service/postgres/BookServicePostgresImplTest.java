...
class BookServicePostgresImplTest {

    @Mock
    private BookPostgresRepository bookRepository;

    @InjectMocks
    private BookServicePostgresImpl bookService;
    ...
    void createBookSuccessfully() {
        BookPostgres book = new BookPostgres((long) 1, "Title", "Author", "ISBN", true);
        BookDto bookDto = new BookDto((long) 1, "Title", "Author", "ISBN", true);
        when(bookRepository.save(any(BookPostgres.class))).thenReturn(book);
    ...
    @Test
    void listAllBooks() {
        List<BookPostgres> books = List.of(new BookPostgres((long) 1, "Title1", "Author1", "ISBN1", true));
